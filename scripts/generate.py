...
.moon-peek { position: fixed; bottom: -40px; left: -40px; width: 90px; height: 90px; border-radius: 50%; z-index: -2; pointer-events: none; opacity: 0.3; filter: blur(0.5px); background: radial-gradient(circle at 35% 30%, color-mix(in srgb, var(--text) 40%, transparent), transparent 75%); animation: floatMoon 13s ease-in-out infinite; }
@keyframes floatMoon { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(10px); } }
@media (prefers-reduced-motion: reduce) { .moon-peek { animation: none; } .warp-overlay.active { animation: none; } }
@media print {
